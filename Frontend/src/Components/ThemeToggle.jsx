...
export default function ThemeToggle() {
    ...
    const toggleTheme = () => {
        ...
        // 3. Determine new theme (switch between light/dark)
        const newTheme = currentTheme === 'dark' ? 'light' : 'dark';

        // 4. Apply new theme to HTML
        htmlElement.setAttribute('data-theme', newTheme);

        // 5. Save user preference to browser storage
        localStorage.setItem('theme', newTheme);
    };

    return (
        <div>
            {/* DaisyUI toggle button */}
            <button
                onClick={toggleTheme}  // Call toggle function on click
                className="btn btn-ghost btn-circle"  // Styled as ghost button
                aria-label="Toggle theme"  // Accessibility label
            >
                {/* Sun icon - shown in LIGHT theme */}
                <svg
                    className="w-5 h-5 fill-current block dark:hidden"  // Hide in dark mode
